# HISTORIAL DE TRABAJO

## Frontend
- 27-04-2026: Maquetacion MIRO (LINK: ).
- 28-04-2026: Organización de carpetas, elaboracion index principal.
- 29-04-2026: Estuctura Sing Page App (formulario).
- 2-05-2026: Restuccturación de formulario = desplegable instituto (Dependencia intituto localidades).
- 2-05-2026: Cambiar roles al rellenar formulario (usuario - administrador).
- 5-05-2026: Pruba de conexion frontend-backend correcta (guarda correctamente).
- 6-05-2026: Pruba de conexion frontend-backend correcta (muestra y elimina).
- 6-05-2026: configuracion del pane de logeo (solo se puede eliminar haciendo login).
- 6-05-2026: usuario solo puede ver la lista.
- 7-05-2026: solo admin puede ver eliminar.
- 8-05-2026: Comentarios.


## Backend
- 30-04-2026: Diseño de la base de datos (Universo de discurso).
- 30-04-2026: organizacion de las conexiones (PHP-BBDD)
- 2-05-2026: Diseño de base de datos (Esquema Entidad-Relacion y modelo relacional) (LINK: )
- 2-05-2026: organizacion de las conexiones
- 3-05-2026: Creacion de la base de datos en Heidisql.
- 4-05-2026: Editar guardar.php para ver si en verdad la se guarda el formulario.
- 5-05-2026: Pruba de conexion frontend-backend correcta (guarda correctamente).
- 6-05-2026: backend mostrar y eliminar.
- 7-05-2026: solo admin puede eliminar.
- 8-05-2026: comentarios.


- 9-05-2026: archivo readme y sus correspondientes imagenes.

